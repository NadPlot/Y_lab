...
def test_get_dish_not_exists(clear_menus):
    ...
    """
    response = client.get(
        f'/api/v1/menus/{test_id}/submenus/{test_id}/dishes/{test_id}',
    )
    assert response.status_code == 404
...
def test_update_submenu(menu_id, submenu_id):
    ...
    response = client.patch(
        f'/api/v1/menus/{menu_id}/submenus/{submenu_id}/', json=data,
    )
    assert response.status_code == 200
    menu = response.json()
    assert menu['title'] == data['title']
...
def test_add_dish(menu_id, submenu_id):
    ...
    data = {
        'title': 'Test Dish',
        'description': 'Test description Dish', 'price': '12.551',
    }
...
def test_update_dish(menu_id, submenu_id, dish_id):
    ...
    data = {
        ...
        'description': 'Test Update description Dish', 'price': '12.4421',
    }
    response = client.patch(
        f'/api/v1/menus/{menu_id}/submenus/{submenu_id}/dishes/{dish_id}', json=data,
    )
    assert response.status_code == 200
    dish = response.json()
    assert dish['title'] == data['title']
